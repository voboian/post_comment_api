...
def generate_reply(post_content, comment_content):
    ...
    reply_text = reply_text.split("Comment:")[0].strip()  # Витягуємо тільки текст до наступного коментаря
    reply_text = reply_text.split("Reply:")[0].strip()  # Витягуємо тільки текст до наступної відповіді
...
def comments_daily_breakdown(request, date_from: str, date_to: str):
    ...
    if date_from > date_to:
        return JsonResponse({"error": "'date_from' must be earlier than 'date_to'."}, status=400)
    ...
    while current_date <= date_to.date():
        day_stats = next((d for d in daily_stats if d["created_at__date"] == current_date), None)
        if day_stats:
            result.append({
